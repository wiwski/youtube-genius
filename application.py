...
def getRelated(videoIds, maxResults=30, underground=False):
    ...
    for vId in videoIds:
        ...
        if underground is True:
            ...
            related = youtube.search().list(part='snippet', maxResults=maxResults,order='relevance', relatedToVideoId=videoId, type='video', pageToken=nextPageToken).execute()
...
def onlyUnderground(videos, vId, whatsUnderground = 100000, nextPageToken=None, sumVideo=0, deep=-1):
    list_ids = []
    ...
    undergroundVideos = [(statVideo['id'] , int(statVideo['statistics']['viewCount'])) for statVideo in statVideos['items'] if int(statVideo['statistics']['viewCount']) < whatsUnderground]
    ...
    while True:
        ...
        if 'nextPageToken' not in videos:
            ...
            deep += 1
            undergroundVideos.extend(onlyUnderground(videos=None, vId=undergroundVideos[deep][0], nextPageToken=None, sumVideo=sumVideo, deep=deep))
            sumVideo=len(undergroundVideos)
            continue
        undergroundVideos.extend(onlyUnderground(videos=None, vId=vId, nextPageToken=videos['nextPageToken'], sumVideo=sumVideo))
        sumVideo = len(undergroundVideos)
